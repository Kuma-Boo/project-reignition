...
public partial class PlayerStatsController : Node
{
	...
	[ExportSubgroup("Ground Settings")] // Default ground settings
	[Export] public int baseGroundSpeed;
	[Export] private int baseGroundTraction;
	...
	[Export(PropertyHint.Range, "1,2,.1f")] private float groundSpeedHighRatio = 1.6f;
	[Export(PropertyHint.Range, "1,5,.1f")] private float tractionLowRatio = 1.2f;
	...
	[ExportSubgroup("Air Settings")] // Default air settings
	[Export] private int baseAirSpeed;
	[Export] private int baseAirTraction;
	[Export] private int baseAirFriction;
	...
	[ExportSubgroup("Slide Settings")] /// <summary> Slide speed when starting from a standstill. </summary>
	[Export] public int InitialSlideSpeed { get; private set; }
	// Default slide settings
	[Export] private int baseSlideSpeed;
	[Export] private int baseSlideTraction;
	[Export] private int baseSlideFriction;
	[Export] private int baseSlideOverspeed;
	[Export] private int baseSlideTurnaround;
	[Export(PropertyHint.Range, "0,1,.1f")] private float slideDistanceLowFrictionRatio = .6f;
	[Export(PropertyHint.Range, "0,1,.1f")] private float slideDistanceMediumFrictionRatio = .4f;
	[Export(PropertyHint.Range, "0,1,.1f")] private float slideDistanceHighFrictionRatio = .2f;
	[Export(PropertyHint.Range, "0,1,.1f")] private float slideSlopeSpeedInfluence = .5f;
	/// <summary> How much should the steepest slope affect the player? </summary>
	[Export] public float slopeInfluence = 1f;
	...
	[ExportGroup("Static Action Settings")][Export] private int baseBackflipSpeed;
	[Export] private int baseBackflipTraction;
	[Export] private int baseBackflipFriction;
	[Export] private int baseBackflipOverspeed;
	[Export] private int baseBackflipTurnaround;
	[Export] private int baseBackstepSpeed;
	[Export] private int baseBackstepTraction;
	[Export] private int baseBackstepFriction;
	[Export] private int baseBackstepOverspeed;
	[Export] private int baseBackstepTurnaround;
	[Export] public float JumpHeight { get; private set; }
	[Export] public float DoubleJumpHeight { get; private set; }
	[Export] public float DoubleJumpSpeed { get; private set; }

	[ExportGroup("Grind Settings")][Export] public float perfectShuffleSpeed;
	[Export] private int baseGrindSpeed;
	[Export] private int maxBonusGrindSpeed;
	[Export] private int baseGrindFriction;
	[Export] private int baseGrindTurnaround;
	[Export(PropertyHint.Range, "0,1,.1f")] private float grindLowRatio = 0.2f;
	[Export(PropertyHint.Range, "0,1,.1f")] private float grindMediumRatio = 0.5f;
	[Export(PropertyHint.Range, "0,1,.1f")] private float grindHighRatio = 1f;
	...
	[ExportGroup("Sidle Settings")][Export] public Curve sidleMovementCurve;

	// References to the actual movement settings being used
	public MovementSetting GroundSettings { get; private set; }
	public MovementSetting StrafeSettings { get; private set; }
...
public class MovementSetting
{
	...
	/// <summary> Interpolates between speeds based on a (signed) input. </summary>
	public float UpdateInterpolateSigned(float currentSpeed, float input)
	{
		float delta = Traction;
		float targetSpeed = Speed * input;
		...
		if (input == 0) // Deccelerate
			delta = Friction;
		else if (!Mathf.IsZeroApprox(currentSpeed) && Mathf.Sign(currentSpeed) != Mathf.Sign(input)) // Turnaround
			delta = Turnaround;
