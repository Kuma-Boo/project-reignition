...
namespace Project.Gameplay
{
	...
	public class JumpPanel : Launcher
	{
		...
		public override Vector3 CalculatePosition(float t)
		{
			...
			Vector3 middlePoint = this.Up() * travelHeight + this.Forward() * travelDistance * .5f + this.Right() * hOffset * .5f;
			Vector3 position = (middlePoint * t).LinearInterpolate(GetEndPoint() * t, t);
			return GetStartingPoint() + position;
		}

		protected override Vector3 GetEndPoint() => this.Forward() * travelDistance + this.Up() * endingHeight + this.Right() * hOffset;
	}
}
