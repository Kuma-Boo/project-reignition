...
uniform float highlight_intensity : hint_range(0f, 8f);
...
void fragment()
{
	...
	float highlight_mask = texture(TEXTURE, UV - vec2(highlight_intensity, 0f) * TEXTURE_PIXEL_SIZE).a;
	highlight_mask *= texture(TEXTURE, UV + vec2(highlight_intensity, 0f) * TEXTURE_PIXEL_SIZE).a;
	highlight_mask *= texture(TEXTURE, UV - vec2(0f, highlight_intensity) * TEXTURE_PIXEL_SIZE).a;
	highlight_mask *= texture(TEXTURE, UV + vec2(0f, highlight_intensity) * TEXTURE_PIXEL_SIZE).a;
	highlight_mask = clamp(highlight_mask, 0f, 1f);
